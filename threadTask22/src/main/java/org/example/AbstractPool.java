...
public abstract class AbstractPool<T> {
    ...
    public AbstractPool(
            final Supplier<T> objectSupplier, final int size) {
    ...
    private synchronized T acquiredObject() {
        return this.poolObjects.stream()
                ...
                .findFirst()
                .map(AbstractPool::markIsIssued)
                .map(PoolObject::getObject)
                .orElseThrow(IllegalStateException::new);
    }

    private static <T> PoolObject<T> markIsIssued(
            final PoolObject<T> poolObject) {
    ...
    private synchronized boolean releaseObject(final T object) {
        return this.poolObjects.stream()
                ...
                .filter(poolObject -> Objects.equals(
                        poolObject.getObject(),
                        object
                )).findFirst().map(this::cleanPoolObject).isPresent();
    }

    private PoolObject<T> cleanPoolObject(
            final PoolObject<T> poolObject) {
